...
def call_gemini_expert_exhaustive(prompt, context, timeout_val=60, image_obj=None):
    ...
    if not context: 
        full_prompt = f"{SYSTEM_PROMPT_EXPERT}\n\n=== TÀI LIỆU HỖ TRỢ ===\n(Hiện tại không tìm thấy tài liệu nào khớp với câu hỏi)\n\n=== CÂU HỎI ===\n{prompt}\n\nLƯU Ý CỐT TỬ: Nếu đây là câu chào hỏi, hãy chào lại. Nếu là câu hỏi nghiệp vụ PCCC, BẮT BUỘC trả lời đúng nguyên văn: 'Xin lỗi đồng chí, tôi chưa tìm thấy quy định cụ thể về vấn đề này. Bạn có thể liên hệ Đại úy Phạm Tùng Linh để cập nhật quy định này cho tôi nhé, cảm ơn bạn.' TUYỆT ĐỐI KHÔNG sử dụng kiến thức trên mạng để trả lời."
    else: 
        ...
        with st.spinner("🧠 Đang suy nghĩ, bạn chờ chút, mình trả lời ngay đây..."):
            ...
            if not relevant_context:
                for fname, content in database.items():
                    ...
                    is_tech_06 = any(kw in prompt_lower for kw in ["khoảng cách", "ngăn cháy", "thông gió", "hút khói", "chống cháy lan", "đường", "bãi đỗ", "vật liệu", "kích thước", "lối thoát", "06", "qc06"]) and not is_tech_10 and not is_penalty
